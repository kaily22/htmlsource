//userid : 영문 대소문자, 숫자, 길이 6~12, 무조건 입력되어야
//password : 영문 대소문자, 숫자, 특수문자(!@#$%*), 길이 8~16, 무조건 입력되어야
//confirm_password : 영문 대소문자, 숫자, 특수문자(!@#$%*), 길이 8~16, 무조건 입력되어야
//                  password와 동일해야 함
//name : 2~4자리 허용, 한글
...
//form submit 되면 form_validate()와 연결 //onclick()대신 이렇게 처리 addEventListener
document.querySelector("form").addEventListener('submit',form_validate);

function form_validate(event){
    event.preventDefault();//submit되지 않도록 이벤트 제어
    ...
    if(!userid_reg.test(userid.value)){
        document.querySelector("small[id='userid']").innerHTML="아이디를 확인해 주세요";
    }else{
    ...
    if(!password_reg.test(password.value)){
        document.querySelector("small[id='password']").innerHTML="비밀번호를 확인해 주세요";
    }else{
        document.querySelector("small[id='password']").innerHTML="";
    }

    if(!password_reg.test(confirm_password.value)){
        document.querySelector("small[id='confirm_password']").innerHTML="비밀번호를 확인해 주세요";
    }else{
        if(password.value !== confirm_password.value){
            document.querySelector("small[id='confirm_password']").innerHTML="이전 비밀번호와 다릅니다";
        }else{
    ...
    if(!name_reg.test(name.value)){
        document.querySelector("small[id='name']").innerHTML="이름을 확인해주세요";
    }else{
    ...
    if(!checked){
        document.querySelector("small[id='gender']").innerHTML="성별을 확인해 주세요";
    }else{
    ...
    if(!email_reg.test(email.value)){
        document.querySelector("small[id='email']").innerHTML="이메일을 확인해주세요";
    }else{
