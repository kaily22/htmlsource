...
$(function (){
    ...
    e.preventDefault();//submit되지 않도록 이벤트 제어
    ...
    if(!userid_reg.test(userid.val())){
        $("small[id='userid']").html("아이디를 확인해 주세요");
    }else{
    ...
    if(!password_reg.test(password.val())){
        $("small[id='password']").html("비밀번호를 확인해 주세요");
    }else{
        $("small[id='password']").html("");
    }

    if(!password_reg.test(confirm_password.val())){
        $("small[id='confirm_password']").html("비밀번호를 확인해 주세요");
    }else{
        if(password.value !== confirm_password.val()){
            $("small[id='confirm_password']").html("이전 비밀번호와 다릅니다");
        }else{
    ...
    if(!email_reg.test(email.val())){
        $("small[id='email']").html("이메일을 확인해주세요");
    }else{
        $("small[id='email']").html("");
    }
   })
